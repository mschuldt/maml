  ...
  (if (string-match "\{\\(.+\\)\}" str)
    ...
    (dolist (x '(":" "->"))
      (if (string-match x str)
          (setq str (car (last (split-string str x))))))
    (setq str (s-trim str))
    (let ((start 0)
      ...
      (while (and (< start end) (not done))
        ...
        (if (not (cond ((eq s ?\[)
            ...
            (setq done t)))
      (if ok (py-str str)
        (message "SyntaxError: invalid parameter type: '%s'" str)
        ;;(kill-emacs 1)
        ))))
;;(to-py-tuple (extract-type "{int, str, [int], (str)}"))
  ...
  (let (arg-types
    ...
    (dolist (arg (mapcar 's-trim (split-string str "," t)))
      ...
      (if (return-type-p arg)
          (if ret ;;there can only be one return type
              (progn
    ...
    (cons (to-py-tuple (reverse arg-types)) (or ret "'none'"))))

(defun py-str (x)
  (format "'%s'" x))

(defun to-py-tuple (lst)
  (format "(%s%s)" (mapconcat 'identity lst ", ")
          (if (= (length lst) 1) "," "")))
